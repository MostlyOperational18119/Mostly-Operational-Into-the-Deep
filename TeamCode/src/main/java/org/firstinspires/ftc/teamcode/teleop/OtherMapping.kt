...
@TeleOp(name = "OtherMapping", group = "AAAA")
class OtherMapping: DriveMethods() {
...
//        setMotorModeEncoder(tapeMeasureRotateMotor)
        ...
        var reverseDriveToggle = false
        var clawServoToggle = false
        ...
        while (opModeIsActive()) {
            ...
            //REVERSE
            if (controller1.left_bumper && !previousController1.left_bumper && !reverseDriveToggle){ reverseDrive = -1;reverseDriveToggle = true }
            else if (controller1.left_bumper && !previousController1.left_bumper && reverseDriveToggle){ reverseDrive = 1;reverseDriveToggle = false }
...
//          }
            ...
            when (automatedTransferToggle) {
                AutomaticTransferState.Pickup -> {
                    //TRANSFER TOGGLE
                    if (controller2.dpad_right && !previousController2.dpad_right){ automatedTransferToggle = AutomaticTransferState.Transfer }
                    ...
                    // CLAW SERVO
                    if (controller2.right_trigger > 0.5 && !clawServoToggle){ clawServo.position = clawServoClosed; clawServoToggle = true }
                    else if (controller2.right_trigger > 0.5 && clawServoToggle) { clawServo.position = clawServoOpen;clawServoToggle = false }

                    //HANGING
                    if (controller1.y && !previousController1.y){hangerMotor.targetPosition = 2000; hangerMotor.power = 0.2}
                    if (controller1.a && !previousController1.a){hangerMotor.targetPosition = 0; hangerMotor.power = -0.2}
                    if (controller1.x && !previousController1.x){hangPusher.position = servoHangActive}
                    ...
                    if (controller2.b && !previousController2.b){ transferServo.position  = clawRotateWall }

                    //HORIZONTAL OUT
                    if (controller2.left_stick_button && !previousController2.left_stick_button) { transferServo.position = transferUpPos; horizontalSlideToggle = HorizontalSlideState.Extend }
                    ...
                    //VERTICAL SLIDE
                    if (controller2.dpad_left  && !previousController2.dpad_left) { verticalSlideToggle = VerticalSlideState.Bar; clawRotateServo.position = clawRotateStraight }
                    if (controller2.dpad_up  && !previousController2.dpad_up) { verticalSlideToggle = VerticalSlideState.High; clawRotateServo.position = clawRotateOut }
                    if (controller2.dpad_down  && !previousController2.dpad_down) { verticalSlideToggle = VerticalSlideState.Floor; clawRotateServo.position = clawRotateUpRight }
