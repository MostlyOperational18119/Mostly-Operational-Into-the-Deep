...
@TeleOp(name = "Meet2DONTUSE", group = "Aardvark")
class Meet2Goodtest: DriveMethods() {
...
//        setMotorModeEncoder(tapeMeasureRotateMotor)
        ...
        val transferThread = thread(true) {
            telemetry.addLine("threaded1")
            Log.i("threaded", "yay")
            if (automatedTransferToggle == AutomaticTransferState.Transfer) {
                telemetry.addLine("threaded2")
                if (singleRunCheck == 1) {
                    singleRunCheck = 2
                    intakeServo?.power = 0.0
                    slideHorizontalMotor.targetPosition = 0
                    slideHorizontalMotor.power = -0.8
                    slideVerticalMotor.targetPosition = 300
                    if (slideVerticalMotor.currentPosition > slideVerticalMotor.targetPosition - 50 && slideVerticalMotor.currentPosition < slideVerticalMotor.targetPosition + 50) {
